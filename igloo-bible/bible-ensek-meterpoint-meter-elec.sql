...
-- Meterpoint Attributes Elec Indicators
       max(case
             when max(case
                        when (mta_elec.metersattributes_attributename = 'MeterType' and
                              mta_elec.metersattributes_attributevalue is not null) then 1
                        else 0
                          end) > 0 then 1
             else 0 end) over (partition by su.external_id) as has_meter_attribute_elec_meter_type,
       max(case
             when max(case
                        when (mta_elec.metersattributes_attributename = 'Meter_Location' and
                              mta_elec.metersattributes_attributevalue is not null) then 1
                        else 0
                          end) > 0 then 1
             else 0 end) over (partition by su.external_id) as has_meter_attribute_elec_meter_location,
       max(case
