select su.external_id                                       as account_id,
    ...
    -- Meterpoint Meters Registers Attributes  Electricity Section
       max(case
             when rga_elec.registersattributes_attributename = 'Measurement_Quantity_ID' then rga_elec.registersattributes_attributevalue
               end)                                         as meterpoint_register_elec_measurement_quantity_id,
       max(case
             when rga_elec.registersattributes_attributename = 'Multiplier' then rga_elec.registersattributes_attributevalue
               end)                                         as meterpoint_register_elec_multiplier,
       max(case
             when rga_elec.registersattributes_attributename = 'No_Of_Digits' then rga_elec.registersattributes_attributevalue
               end)                                         as meterpoint_register_no_of_digits,
       max(case
             when rga_elec.registersattributes_attributename = 'Type' then rga_elec.registersattributes_attributevalue
...
-- Meterpoint Attributes Elec Indicators
       max(case
             when max(case
                        when (rga_elec.registersattributes_attributename = 'Measurement_Quantity_ID' and
                              rga_elec.registersattributes_attributevalue is not null) then 1
                        else 0
                          end) > 0 then 1
             else 0 end) over (partition by su.external_id) as has_meterpoint_register_elec_measurement_quantity_id,
       max(case
             when max(case
                        when (rga_elec.registersattributes_attributename = 'Multiplier' and
                              rga_elec.registersattributes_attributevalue is not null) then 1
                        else 0
                          end) > 0 then 1
             else 0 end) over (partition by su.external_id) as has_meterpoint_register_elec_multiplier,
       max(case
             when max(case
                        when (rga_elec.registersattributes_attributename = 'No_Of_Digits' and
                              rga_elec.registersattributes_attributevalue is not null) then 1
                        else 0
                          end) > 0 then 1
       ...
       max(case
             when max(case
                        when (rga_elec.registersattributes_attributename = 'Type' and
                              rga_elec.registersattributes_attributevalue is not null) then 1
                        else 0
                          end) > 0 then 1
...
from ref_cdb_supply_contracts su --inner join temp_cab_dates_quarter4_2018 we on we.external_id = su.external_id
       ...
       left outer join ref_meters_attributes mta_elec
         on mt_elec.meter_id = mta_elec.meter_id
