drop table ref_calculated_tado_efficiency_reporting;

create table ref_calculated_tado_efficiency_reporting
  as
    ...
    SELECT rcte.user_id                                         as user_id,
           ...
           max(case
                 when max(case
                            when (mta_gas.metersattributes_attributename = 'MeterType' and
                                  mta_gas.metersattributes_attributevalue in ('S1', 'S2', 'NS')) then 1
                            else 0
                              end) > 0 then 1
                 else 0 end) over (partition by su.external_id) as has_smart_gas_meter,
           rcte.base_temp,
           rcte.heating_basis,
           ...
           rcte.base_mit,
           rcte.estimated_mit,
           rcte.est_annual_source,
           rcte.est_annual_fuel_used,
           rcte.igloo_avg_perc_diff,
           rcte.igloo_avg_savings,
           rcte.unit_rate_with_vat,
           rcte.amount_over_year,
           rcte.perc_diff,
           rcte.savings_in_pounds,
           rcte.segment                                         as segment,
           rca.postcode                                         as supply_postcode,
           getdate()                                            as etlchange
    FROM ref_calculated_tado_efficiency_batch rcte
           inner join ref_cdb_supply_contracts su
             on rcte.account_id = su.external_id --inner join temp_cab_dates_quarter4_2018 we on we.external_id = su.external_id
           ...
           left outer join ref_meterpoints mp_elec
             on mp_elec.account_id = su.external_id and mp_elec.meterpointtype = 'E'
           ...
           left outer join ref_meters_attributes mta_gas on mt_gas.meter_point_id = mta_gas.meter_point_id and
