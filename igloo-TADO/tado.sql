...
select x1.user_id,
       ...
       ((
          ...
          tado_estimate_mean_internal_temp(coalesce(x1.base_temp_used, 0.0), coalesce(x1.base_hours, 0.0))
            ) * est_annual_fuel_used * ((unit_Rate + (unit_Rate * .05)) / 100))                                                                       savings_in_pounds,
...
from (
      select x.user_id,
             x.account_id,
             x.base_temp,
             case when x.base_temp<=0 then x.default_base_temp
                                                else x.base_temp end as base_temp_used,
             case when heating_control_type='' then 'unknown' else x.heating_control_type end     as heating_control,
             case when x.heating_basis ='' then 'unknown' else x.heating_basis end                as heating_basis,
             case when x.family_category ='' then 'unknown' else x.family_category end            as family_category,
             ...
             tado_estimate_setpoint_impact(case when x.base_temp<=0 then x.default_base_temp
    ...
    -- Model 1st Level Inputs
      ...
      select       u.id                      as user_id,
                  ...
                  coalesce((select sum(reg.registers_eacaq)
                     ...
                     where mp.account_id = su.external_id and mp.meterpointtype = 'G'
                                                     and (mp.supplyenddate is null or mp.supplyenddate >= getdate())
                     group by mp.account_id
                  ), 0) as aq,
                  coalesce((select q.gas_usage from ref_cdb_quotes q  where q.user_id = u.id order by updated_at desc limit 1), 0) as gas_usage
