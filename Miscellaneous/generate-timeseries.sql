...
create view  vw_timeseries_48hours
  as
select cast(trunc(getdate()-1) as timestamp) + (i * interval '1 sec') as time
from  generate_series(1, (172800 - 1)) i;
...
from (select cast(trunc(getdate() - 1) as timestamp) + (i * interval '1 sec') as time
      from generate_series(1, (172800 - 1)) i) t
       left outer join dwh_job_logs l on l.job_start = t.time
...
SELECT l.job_id,
       l.job_name,
       l.job_script_name,
       to_char(l.job_start,YYY)HH24:MI:SS) as job_start,
       l.job_end,
       l.job_status,
       LTRIM(DATEADD(seconds, DATEDIFF(second, job_start, nvl(job_end, getdate())), '1900-01-01 00:00:00'),
             '1900-01-01') as run_time
