...
, cte_credits as (
    ...
    case when ref.Refunds > ensek.Credits then ref.Refunds - ensek.Credits else null end as "In GC not in Ensek" ,
    case when ref.Refunds < ensek.Credits then ensek.Credits - ref.Refunds else null end as "In Ensek not in GC"
    from
    cte_date cd
    left join
        (
 ...
 select * from cte_credits order by 1;
