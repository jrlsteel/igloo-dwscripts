...
, cte_debits as (
    select
    go.gc_date,
    case when coalesce(go.Payments, 0) > coalesce(en.Debits,0) then go.Payments - en.Debits else null end as "In GC not in Ensek" ,
    case when coalesce(go.Payments, 0) < coalesce(en.Debits,0) then en.Debits - go.Payments else null end as "In Ensek not in GC"
    from
...
, cte_credits as (
    select
    go.gc_date,
    case when coalesce(go.Refunds, 0) > coalesce(en.Credits,0) then coalesce(go.Refunds, 0) - coalesce(en.Credits, 0) else null end as "In GC not in Ensek" ,
    case when coalesce(go.Refunds, 0) < coalesce(en.Credits,0) then coalesce(en.Credits, 0) - coalesce(go.Refunds, 0) else null end as "In Ensek not in GC"
    from
    cte_goCardless_rp go
    left join cte_ensek_rp en
        on en.ensek_date = go.gc_date
