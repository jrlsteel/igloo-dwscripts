select x1.account_id,
                            ...
                            from (select account_id,
                                     ...
                                     (ind_eac * (cast(days_since_last_read as double precision) / 365)) as ind_estimated_advance
                              from (select su.external_id                                               as account_id,
                                          ...
                                          coalesce(nullif(eac.igl_ind_eac, 0), nullif(r.registers_eacaq, 0), 0) as eac,
                                           (select sum(ppc_sum)
                                            ...
                                            where gsp_group_id = rma_gsp.attributes_attributevalue
                                              ...
                                              and st_date >= trunc(min(reads.meterreadingdatetime))
                                              and st_date < trunc(max(reads.meterreadingdatetime))
                                    ...
                                    from ref_cdb_supply_contracts su
                                           ...
                                           left outer join ref_meterpoints_attributes rma_gsp
                                             on rma_gsp.account_id = su.external_id and rma_gsp.meter_point_id = mp.meter_point_id and
                                                rma_gsp.attributes_attributename = 'GSP'
                                           left outer join ref_meterpoints_attributes rma_ssc
                                             on rma_ssc.account_id = su.external_id and mp.meter_point_id = rma_ssc.meter_point_id and
                                                rma_ssc.attributes_attributename = 'SSC'
                                           left outer join ref_meterpoints_attributes rma_pcl
                                             on rma_pcl.account_id = su.external_id and mp.meter_point_id = rma_pcl.meter_point_id and
                                                rma_pcl.attributes_attributename = 'Profile Class'
                                           inner join ref_meters m on m.account_id = su.external_id and m.meter_point_id = mp.meter_point_id and
                                           ...
                                           inner join (select *
                                                       from (select account_id,
                                                                    ...
                                                                    row_number() over (partition by ri.account_id, register_id order by meterreadingdatetime desc) rownum
