...
from ref_cdb_surveys cs
         ...
         inner join ref_cdb_user_permissions up
                    on csr.user_id = up.user_id and up.permissionable_type = 'App\\SupplyContract'
         ...
         inner join (select json_objects.id,
                            ...
                            case
                                when json_objects.entity_type = 'App\\Address'
                                    then json_extract_path_text(json_objects.elements, 'name') end         as machine_name,
                            case
                                when json_objects.entity_type = 'App\\Address'
                                    then json_extract_path_text(json_objects.elements, 'display_name') end as machine_display_name,
                            case
                                when json_objects.entity_type = 'App\\Address'
                                    then json_extract_path_text(json_objects.elements, 'usage') end        as machine_usage,
                            case
                                when json_objects.entity_type = 'App\\Address'
                                    then json_extract_path_text(json_objects.elements, 'age') end          as machine_age,
                            case
                                when json_objects.entity_type = 'App\\User'
                                    then json_extract_path_text(json_objects.elements, 'age') end          as user_age
                     from (
                              SELECT id,
                                     entity_type,
                                     JSON_EXTRACT_ARRAY_ELEMENT_TEXT(replace(attribute_custom_value, 'null', '""'),
                                                                     seq.i - 1) AS elements
                              FROM ref_cdb_attributes,
                                   seq_0_to_10 AS seq
                              WHERE seq.i < JSON_ARRAY_LENGTH(replace(
                                      replace(replace(isnull(attribute_custom_value, '[]'), 0, '[]'), 'null', '""'),
                                      '"""', ''))
                                and attribute_custom_value is not null
                                and entity_type in ('App\\User', 'App\\Address', 'App\\SupplyContract')
                              UNION ALL
                              SELECT id, entity_type, attribute_custom_value AS elements
                              FROM ref_cdb_attributes
                              WHERE attribute_custom_value is not null
                                and entity_type in ('App\\Bookings\\Models\\BookingAppointment')
