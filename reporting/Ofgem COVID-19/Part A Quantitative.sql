create or replace view vw_ofgem_covid19_v2 as
    with unused as (select null),
         cte_date_range as (
             select dateadd(second, -1, date)      as month_end,
                    date_trunc('month', month_end) as month_start
             from public.ref_date
             where day = 1
               and date between '2020-04-01' and '2020-04-01' --getdate()
         ),
         cte_payment_in_month as (
         ...
         cte_account_figures as (
             select distinct dcf.account_id,
                             ...
                             dd_amended and current_dd < proposed_dd                                      as active_financial_relief,
                             case
                                 when active_financial_relief then proposed_dd - current_dd
             ...
             from cte_distinct_customer_file dcf
                      left join cte_date_range date_range
                                on true
                      left join cte_payment_in_month cte_pim
