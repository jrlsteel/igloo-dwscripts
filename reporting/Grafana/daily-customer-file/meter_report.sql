-- V1.2.0
drop table temp_mr_111;
create table temp_mr_111 as
with cte_readings as (select rmp.account_id,
                             rmp.meter_point_id,
                             met.meter_id,
                      ...
                      from ref_meterpoints rmp
                               ...
                               left join (select account_id,
                                          ...
                                          from (select account_id,
                                                ...
                                                where meterreadingtypeuid in ('ACTUAL', 'FINAL', 'INITIAL')) readings
                                          where rn = 1
                      ...
                      group by rmp.account_id, rmp.meter_point_id, met.meter_id, read_info.reading_date,
                               read_info.dc_read),
     cte_current_mp_attr as (select account_id,
                                    meter_point_id,
     ...
     cte_current_meters_attr as (select account_id,
                                        meter_point_id,
                                        meter_id,
...
select distinct mp.meterpointnumber                                                           as MPR,
                ...
                rma_lsp.attributes_attributevalue                                             as lsp_flag,
                mp_audit.first_seen                                                           as meterpoint_first_seen,
                rma_pcl.attributes_attributevalue                                             as profile_class,
                reg_tpr.tpr_list                                                              as time_pattern_regime,
...
from ref_meterpoints mp
         ...
         left join cte_current_meters_attr rma_mech_gmm
         ...
         left join cte_current_meters_attr rma_mech_mmc
         ...
         left join cte_current_mp_attr rma_digits_elec
         ...
         left join cte_current_mp_attr rma_digits_gas
         ...
         left join cte_current_mp_attr rma_ssc on rma_ssc.attributes_attributename = 'SSC' and
                                                  rma_ssc.meter_point_id = met.meter_point_id and
                                                  rma_ssc.account_id = met.account_id
         left join cte_current_mp_attr rma_mop
         ...
         left join cte_current_mp_attr rma_mam
         ...
         left join cte_current_mp_attr rma_osmop
         ...
         left join cte_current_mp_attr rma_osmam
         ...
         left join cte_current_mp_attr rma_sup_stat on rma_sup_stat.account_id = mp.account_id and
                                                       rma_sup_stat.meter_point_id = mp.meter_point_id and
                                                       rma_sup_stat.attributes_attributename = 'Supply_Status'
         left join cte_current_mp_attr rma_imp on rma_imp.account_id = mp.account_id and
                                                  rma_imp.meter_point_id = mp.meter_point_id and
                                                  rma_imp.attributes_attributename = 'Gas_Imperial_Meter_Indicator'
         left join cte_current_mp_attr rma_gain_sup on rma_gain_sup.account_id = mp.account_id and
                                                       rma_gain_sup.meter_point_id = mp.meter_point_id and
                                                       rma_gain_sup.attributes_attributename = 'GAIN_SUPPLIER'
         left join cte_current_mp_attr rma_lsp on rma_lsp.account_id = mp.account_id and
                                                  rma_lsp.meter_point_id = mp.meter_point_id and
                                                  rma_lsp.attributes_attributename = 'Large Site Indicator'
         ...
         left join (select account_id, meter_point_id, min(etlchange) as first_seen
                    from ref_meterpoints_audit
                    group by account_id, meter_point_id) mp_audit
                   on mp_audit.account_id = mp.account_id and
                      mp_audit.meter_point_id = mp.meter_point_id
         left join (select account_id, meter_point_id, meter_id, listagg(distinct registers_tpr, ',') as tpr_list
                    from ref_registers
                    group by account_id, meter_point_id, meter_id) reg_tpr
                   on reg_tpr.account_id = met.account_id and
                      reg_tpr.meter_point_id = met.meter_point_id and
                      reg_tpr.meter_id = met.meter_id
