with full_details as (select sc.external_id                               as account_id,
                             ind_aq,
                             pa_cons_gas                                  as ann_cons_gas,
                             get_best_consumption(igl_ind_aq, ind_aq, pa_cons_gas, quotes_aq,
                                                  'gas')                  as ca_gas_source,
                             case ca_gas_source
                             ...
                             get_best_consumption(igl_ind_eac, ind_eac, pa_cons_elec, quotes_eac,
                                                  'elec')                 as ca_elec_source,
                             case ca_elec_source
                             ...
                             (least(old_elec_dif, old_gas_dif) <= 0.7 or
                              greatest(old_elec_dif, old_gas_dif) >= 1.3) as fail_old_pa,
                             (least(new_elec_dif, new_gas_dif) <= 0.7 or
                              greatest(new_elec_dif, new_gas_dif) >= 1.3) as fail_new_pa,
                             ...
                             ssd_sed.ssd,
                             ssd_sed.sed,
                             latest_reading.most_recent_reading
                      ...
                      from ref_cdb_supply_contracts sc
                               ...
                               left join (select account_id, currentbalance
                                          from (select *,
                                                       row_number() over (partition by account_id order by sourcedate desc) as rn
                               ...
                               left join (select account_id,
                                                 ...
                                                 nullif(
                                                         max(nvl(least(supplyenddate, associationenddate), '2100-01-01')),
                                                         '2100-01-01')                                as sed
                               ...
                               left join (select account_id, max(meterreadingdatetime) as most_recent_reading
                                         ...
                                         on latest_reading.account_id = sc.external_id)
select account_id,
       first_name,
       last_name,
       email,
