drop table ref_calculated_tado_heating_efficiency;

create table ref_calculated_tado_heating_efficiency
  as
    select x1.user_id,
           x1.account_id,
           x1.base_temp,
           ...
           tado_estimate_mean_internal_temp(coalesce(x1.estimated_temp, 0.0),
                                            coalesce(x1.estimate_hours, 0.0))                                   as est_mit,
           aq                                                                                                   as AQ,
           unit_Rate                                                                                            as unit_rate,
           aq * unit_Rate                                                                                          amount_over_year,
           abs(
             (
    ...
    from (select x.user_id,
                 ...
                 max(heating_control_type)                                     heating_control,
                 max(x.heating_basis)                                          heating_basis,
                 max(x.heating_type)                                           heating_type,
...
-- tado_estimate_setpoint_impact('19',max(heating_control_type),max(x.heating_basis)) as estimated_temp,
                 tado_estimate_heating_hours(max(heating_type), 'base')     as base_hours,
                 tado_estimate_heating_hours(max(heating_type), 'estimate') as estimate_hours
          from (select u.id                      as user_id,
                       ...
                       case
                         ...
                         else '0' end            as temperature_preference,
                       case
                       ...
                       sr.status                 as status,
-- 4 as no_of_people,
-- 'working_kids' as heating_type
-- json_array_length(at.attribute_custom_value) as no_of_people,
                       case
                ...
                where
                    ...
                    ('resident_ages', 'heating_control_type', 'temperature_preference', 'heating_basis')
-- group by u.id, su.external_id
               -- limit 100
               ) x
                 inner join ref_meterpoints mp on mp.account_id = x.account_id and meterpointtype = 'G'
                 inner join ref_meters m on m.account_id = mp.account_id and m.meter_point_id = mp.meter_point_id
                 inner join ref_registers reg on reg.account_id = mp.account_id and reg.meter_id = m.meter_id
                 inner join ref_tariff_history_gas_ur tf on tf.account_id = x.account_id
          group by x.user_id,
                   x.account_id) x1
    order by  x1.user_id, x1.account_id;