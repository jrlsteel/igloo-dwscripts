...
select * from aws_s3_stage2_extracts.stage2_accounttransactions
where account_id = 6296;
...
SELECT l.job_id,
            ...
            from dwh_job_logs l
where trunc(job_start) >= '2019-05-16'
            order by job_start desc;

select
  ta.account_id as account_id,
  max(creationdetail_createddate) over (partition by at.account_id)
from temp_account_id ta
     left outer join ref_cdb_supply_contracts su on su.external_id = ta.account_id
-- inner join ref_account_status ac on ac.account_id = su.external_id
    left outer join aws_s3_stage2_extracts.stage2_accounttransactions at
    on at.account_id = su.external_id


order by ta.account_id;


select external_id, supplystartdate, supplyenddate from ref_cdb_supply_contracts su
inner join ref_meterpoints mp on mp.account_id = su.external_id
    where supplyenddate between '2019-01-01' and '2019-03-31'
      and  supplystartdate < supplyenddate
and supplyenddate is not null;



select account_id,
       max(creationdetail_createddate) from
aws_s3_stage2_extracts.stage2_accounttransactions
group by account_id;

select * from aws_s3_stage2_extracts.stage2_accounttransactions at
where at.iscancelled != 'False' and transactiontype = 'R';
